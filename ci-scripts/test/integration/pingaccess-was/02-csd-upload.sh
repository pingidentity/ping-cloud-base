...
declare -a UPLOAD_CSD_JOB_NAMES=("pingaccess-was-periodic-csd-upload" "pingaccess-was-admin-periodic-csd-upload")
...
actual_files() {
  ...
  aws s3 ls \
    ...
    xargs basename | 
    sort |
    if [[ "$UPLOAD_CSD_JOB_POD_PREFIX" == "pingaccess-was" ]];
    then 
        cat | grep -v "admin"
    else
        cat
...
UPLOAD_JOBS="${PROJECT_DIR}/k8s-configs/ping-cloud/base/pingaccess-was/aws/periodic-csd-upload.yaml"
