...
curl() {
  create_application_422_response=$(cat "${resources_dir}"/create-application-422-response.txt)
  # echo into stdout as a return value
  echo "${create_application_422_response}"
  return 0
}
...
testCreateApplicationNegCase422Response() {
  local http_422='HTTP/1.1 422 Unprocessable Entity'
  local failed="Application with id '2' already exists."
