...
if "${UPDATE_MANIFESTS}"; then
  ...
  echo "      grep 'kind: Secret' /tmp/deploy.yaml # shouldn't have Secrets manifests, except os-securityconfig and karpenter-cert, but could have Secrets within ConfigMap manifests"
