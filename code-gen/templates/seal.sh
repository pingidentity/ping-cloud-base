...
# It requires kustomize, kubeseal, yq and jq to be installed.
...
check_binaries "kustomize" "kubeseal" "yq" "jq"
HAS_REQUIRED_TOOLS=${?}
...
for SECRET in $(kustomize build | yq r -j -d'*' - | jq -rc '.[] | select(.kind == "Secret")'); do
  NAME=$(echo "${SECRET}" | jq -rc '.metadata.name')
  NAMESPACE=$(echo "${SECRET}" | jq -rc '.metadata.namespace')
  DATA=$(echo "${SECRET}" | jq '.data')
  ...
  if test "${DATA}" != 'null'; then
    ...
    echo "${SECRET}" | kubeseal --cert "${CERT_FILE}" -o yaml >> "${SEALED_SECRETS_FILE}"
    echo --- >> "${SEALED_SECRETS_FILE}"
