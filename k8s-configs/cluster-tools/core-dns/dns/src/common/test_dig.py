...
ternary_k8s_domain = 'ping-cloud-tertiary.svc.cluster.local'


logger = core_dns_logging.CoreDnsLogger(False)

def test_get_k8s_domain_to_ip_mappings():
    ...
    domain_to_ips = dig_mgr.get_k8s_domain_to_ip_mappings(namespace, tenant_domain)
    logger.log('return value is')
    logger.log(type(domain_to_ips))
...
domain_to_ips = test_get_k8s_domain_to_ip_mappings()
assert len(domain_to_ips) == 3
