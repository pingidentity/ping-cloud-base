...
NUM_REPLICAS=$(kubectl get statefulset "${K8S_STATEFUL_SET_NAME_PINGACCESS_WAS}" -o jsonpath='{.spec.replicas}')
echo "pre-stop: number of replicas: ${NUM_REPLICAS}"

if test "${ORDINAL}" -lt "${NUM_REPLICAS}"; then
  echo "pre-stop: not removing engine ${ORDINAL} since it is still in the topology"
  exit 0
fi

ADMIN_HOST_PORT="${K8S_SERVICE_NAME_PINGACCESS_WAS_ADMIN}:9000"
