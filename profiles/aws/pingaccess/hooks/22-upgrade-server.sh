...
function process_engine()
{
   rc=0
   #
   # Configure for either Pingaccess or Pingaccess-WAS
   #

   if isPingaccessWas; then
      ADMIN_HOST_PORT="${K8S_SERVICE_NAME_PINGACCESS_WAS_ADMIN}:${PINGACCESS_WAS_ADMIN_SERVICE_PORT_PINGACCESS_WAS_ADMIN}"
   else
      ADMIN_HOST_PORT="${K8S_SERVICE_NAME_PINGACCESS_ADMIN}:${PINGACCESS_ADMIN_SERVICE_PORT}"
   fi
