...
make_initial_api_request -s -X PUT -d "${eula_payload}" "https://localhost:9000/pa-admin-api/v3/users/1" > /dev/null
...
make_initial_api_request -s -X PUT -d "${change_password_payload}" "https://localhost:9000/pa-admin-api/v3/users/1/password" > /dev/null
...
create_config_query_keypair_response=$(make_api_request -s -d "${config_query_keypair_payload}" "https://localhost:9000/pa-admin-api/v3/keyPairs/generate")
...
config_query_response=$(make_api_request -s -X PUT -d "${config_query_payload}" "https://localhost:9000/pa-admin-api/v3/httpsListeners/${config_query_listener_id}")
...
admin_config_response=$(make_api_request -s -X PUT -d "${admin_config_payload}" "https://localhost:9000/pa-admin-api/v3/adminConfig")
