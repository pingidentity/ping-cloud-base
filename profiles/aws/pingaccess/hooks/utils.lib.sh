...
function substitute_kms_key_id() {
  ...
  envsubst "${APP_VARS}" \
    < "${STAGING_DIR}/instance/conf/template/pa.jwk.properties" \
    > "${SERVER_ROOT_DIR}/conf/pa.jwk.properties"
...
{
  SERVER_PID=$( jps -vVlm | grep com.pingidentity.pa.cli.Starter | awk '{print $1}')
  kill "${SERVER_PID}"
  while true; do
    SERVER_PID=$( jps -vVlm | grep com.pingidentity.pa.cli.Starter | awk '{print $1}')
...
function make_api_request() {
    set +x
    http_code=$(curl -sS -k -o ${OUT_DIR}/api_response.txt -w "%{http_code}" \
...
function make_api_request_download() {
    set +x
    http_code=$(curl -k -sS -w "%{http_code}" \
         --retry ${API_RETRY_LIMIT} \
         --max-time ${API_TIMEOUT_WAIT} \
         --retry-delay 1 \
         --retry-connrefused \
