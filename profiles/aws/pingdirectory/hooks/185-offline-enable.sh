...
# Use a config file with no wrapping for searching through the config.ldif file
# FIXME: in a later release, look into using LDIF tools instead of a generic text processing tool such as grep
conf_no_wrap="${tmp_dir}/config.ldif.nowrap"

transform-ldif --sourceLDIF "${conf}" --targetLDIF "${conf_no_wrap}" --redactAttribute doesNotExist
transform_result=$?

if test ${transform_result} -ne 0; then
  beluga_error "Error removing wrapping from ${conf}: ${transform_result}"
  exit 1
fi

beluga_log "Difference between ${conf} and ${conf_no_wrap}"
diff "${conf}" "${conf_no_wrap}"

cat "${header}" "${conf_no_wrap}" > "${conf}"
