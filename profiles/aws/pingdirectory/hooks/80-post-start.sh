...
initialize_server_for_dn() {
  ...
  echo "dsreplication initialize --trustAll --hostSource "${from_host}" --portSource "${from_port}" --useSSLSource --hostDestination "${to_host}" --portDestination "${to_port}" --useSSLDestination --retryTimeoutSeconds "${timeout_seconds}" --baseDN "${base_dn}" --adminUID "${ADMIN_USER_NAME}" --adminPasswordFile "${ADMIN_USER_PASSWORD_FILE}" --no-prompt --ignoreWarnings --enableDebug --globalDebugLevel verbose"
