...
initialize_server_for_dn() {
  ...
  if is_secondary_cluster && test "${ORDINAL}" -eq 0; then
    ...
    local from_service_domain="${PD_CLUSTER_PUBLIC_HOSTNAME}"
  else

    #
    if is_multi_cluster; then
      local from_service_domain="pingdirectory-cluster.${DNS_ZONE}"
    else
      local from_service_domain="${LOCAL_DOMAIN_NAME}"
    fi
  ...
  beluga_log "Verifying that server with ${base_dn} from ${from_running_pod_name} at host ${from_service_domain} replication connection is available"

  local is_base_dn_from_pod_replication_connection_ready=$(ldapsearch \
    --outputFormat values-only \
    --baseDN "cn=monitor" \
    --scope sub "(&(objectClass=ds-replication-server-handler-monitor-entry)(replication-server=${from_running_pod_name}.${from_service_domain}*)(cn=Remote Repl Server ${base_dn}*))" available |\
      tr '[:upper:]' '[:lower:]')

  if [ "${is_base_dn_from_pod_replication_connection_ready}" != "true" ]; then
    beluga_warn "Server with ${base_dn} from ${from_running_pod_name} at host ${from_service_domain} replication connection wasn't available Will wait and try again"
    sleep 15
  ...
  dsreplication initialize \
    ...
    --enableDebug \
    --globalDebugLevel verbose > /dev/null
  replInitResult=$?
