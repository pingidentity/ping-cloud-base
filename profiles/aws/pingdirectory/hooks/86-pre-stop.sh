...
beluga_log "pre-stop: getting instance name from config"
...
beluga_log "pre-stop: removing ${HOSTNAME} (instance name: ${INSTANCE_NAME}) from the topology"
...
beluga_log "pre-stop: server removal exited with return code: ${?}"

beluga_log "pre-stop: removing the replication changelogDb"
...
beluga_log "pre-stop: removing ${POST_START_INIT_MARKER_FILE} and ${REPL_INIT_MARKER_FILE} marker files"
