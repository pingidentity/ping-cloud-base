...
echo "Downloading ${SKBN_CLOUD_PREFIX}/${DATA_BACKUP_FILE_NAME} to ${SKBN_K8S_PREFIX}${SERVER_RESTORE_DIR}"
if ! skbn cp \
  --src "${SKBN_CLOUD_PREFIX}/${DATA_BACKUP_FILE_NAME}" \
  --dst "${SKBN_K8S_PREFIX}${SERVER_RESTORE_DIR}/${DATA_BACKUP_FILE_NAME}"; then
  
  echo "skbn failed to download ${DATA_BACKUP_FILE_NAME} from ${SKBN_K8S_PREFIX}${SERVER_RESTORE_DIR}"
  exit 1
fi
