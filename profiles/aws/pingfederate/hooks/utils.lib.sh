...
function substitute_kms_key_id() {

  beluga_log "Configuring AWS KMS Key"

  export JWK_ENCRYPTED=$1

  APP_VARS='${KMS_KEY_ID}
            ${JWK_ENCRYPTED}'

  envsubst "${APP_VARS}" \
    < "${STAGING_DIR}/templates/com.pingidentity.crypto.jwk.MasterKeySet.xml" \
    > "${OUT_DIR}/instance/server/default/data/config-store/com.pingidentity.crypto.jwk.MasterKeySet.xml"
...
function obfuscatePassword() {
  ...
  cd "${SERVER_ROOT_DIR}/bin"
   ...
   #
#   export PF_LDAP_PASSWORD_OBFUSCATED=$(./obfuscate.sh "${PF_LDAP_PASSWORD}" | tail -2 | tr -d '\n')
   export PF_LDAP_PASSWORD_OBFUSCATED="${PF_LDAP_PASSWORD}"
   #
