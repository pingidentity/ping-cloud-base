...
testAgentConfig() {
  ...
  get_application_response=$(get_application_by_name "" "${PINGACCESS_API}" 'app1' "${TOKEN}")
  ...
  if [[ "${virtual_host_id}" != '' ]]; then
    ...
    delete_virtual_host_response=$(delete_virtual_host "" "${PINGACCESS_API}" "${virtual_host_id}" "${TOKEN}")
  ...
  create_shared_secret_response=$(create_shared_secret "" "${PINGACCESS_API}" "${agent_shared_secret}" "${TOKEN}")
  ...
  # Create virtual host
  create_virtual_host_response=$(create_virtual_host "" "${PINGACCESS_API}" "${TOKEN}")
  ...
  # Create agent
  create_agent_response=$(create_agent "" "${PINGACCESS_API}" ${shared_secret_id} "${pa_engine_host}" "${TOKEN}")
  ...
  # Create application
  create_application_response=$(create_agent_application "" "${PINGACCESS_API}" "${agent_id}" "${virtual_host_id}" "${TOKEN}")
