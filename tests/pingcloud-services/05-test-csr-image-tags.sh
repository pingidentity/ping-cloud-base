...
testOpensearchBootstrapImageTag() {
  ...
  local osBootstrapImage=$(kubectl get pod logstash-elastic-0 -n elastic-stack-logging -o jsonpath='{.spec.initContainers[?(@.name=="opensearch-bootstrap")].image}' | awk -F: '{print $2}')
