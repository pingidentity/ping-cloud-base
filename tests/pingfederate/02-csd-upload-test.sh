...
testPingFederateRuntimeCSDUploadCapturesFailure(){
  init_csd_upload_failure "pingfederate" 5 "${PROJECT_DIR}"/k8s-configs/ping-cloud/base/pingfederate/engine/aws/periodic-csd-upload.yaml
  assertEquals "CSD upload job should not have succeeded" 1 $?
}

testPingFederateRuntimeCSDUploadCapturesFailure(){
  init_csd_upload_failure "pingfederate-admin" 5 "${PROJECT_DIR}"/k8s-configs/ping-cloud/base/pingfederate/admin/aws/periodic-csd-upload.yaml
  assertEquals "CSD upload job should not have succeeded" 1 $?
}
